...
6. **Define your scales.** Fields → Groups & Subgroups: make a `Wellbeing` group holding `WB1:WB5`, a `Digital Stress` group holding `DS*`, and a Demographics group for the background columns. Split a scale into subscales with **+ Sub** if you need to. Fields → Categorise is the flat, quicker alternative. See [Field Groups and Subgroups](/docs/help/field-groups).
